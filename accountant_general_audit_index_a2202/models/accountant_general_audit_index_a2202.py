...
class AccountantGeneralAuditIndexA2202(models.Model):
    _name = "accountant.general_audit_index_a2202"
    _description = "Accountant General Audit Index A.220.2"
    _inherit = [
        "mail.thread",
        "tier.validation",
    ...
    def _compute_policy(self):
        _super = super(AccountantGeneralAuditIndexA2202, self)
    ...
    status_id = fields.Many2one(
        string="Status",
        comodel_name="accountant.general_audit_index_a2202_status",
        required=False,
        states={
            "draft": [
                ("readonly", False),
    ...
    detail_ids = fields.One2many(
        string="Details",
        comodel_name="accountant.general_audit_index_a2202_detail",
        inverse_name="index_a2202_id",
    )
    ...
    def onchange_detail_ids(self):
        self.update({"detail_ids": [(5, 0, 0)]})
        if self.general_audit_id and self.general_audit_id.trial_balance_id:
            result = []
            tb = self.general_audit_id.trial_balance_id
            for detail in tb.computation_ids.filtered(
                lambda r: r.computation_item_id.category != "summary"
            ):
                result.append(
                    (
                        0,
                        0,
                        {
                            "computation_id": detail.id,
                        },
                    )
                )
            self.update({"detail_ids": result})
